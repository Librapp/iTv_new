package com.sumavision.itv.service;
...
public class LocateMySelfService extends Service
{
		...
		public void handleMessage(android.os.Message msg)
		{
			...
			switch (msg.what)
			{
				case MESSAGE_NEW_LOCATION:
					/*
					 * Toast.makeText(LocateMySelfService.this, (String)
					 * msg.obj,
	...
	private void saveLocationInfo(String city)
	{
		...
		OtherCacheData.current().currentCity = city;
		Toast.makeText(getApplicationContext(), OtherCacheData.current().currentCity, Toast.LENGTH_SHORT).show();
		Log.e("LocateMySelfService", OtherCacheData.current().currentCity);

		SharedPreferences sp = getSharedPreferences("otherInfo", Context.MODE_PRIVATE);
		Editor editor = sp.edit();
		editor.putString("currentCity", OtherCacheData.current().currentCity);
		editor.commit();
	}
	...
	private void initLocationInfo()
	{
		...
		SharedPreferences sp = getSharedPreferences("otherInfo", Context.MODE_PRIVATE);
		String tempCity = sp.getString("currentCity", "泉州");
	...
	private void getAddress(String lat, String lon) throws Exception
	{
		...
		URL url = new URL("http://maps.google.cn/maps/geo?key=abcdefg&q=" + lat + "," + lon);
		...
		if (!lines.equals(""))
		{
			...
			for (int i = 0; i < jsonArray.length(); i++)
			{
				...
				// 取出JSON数据中user字段内容
				JSONObject u = jsonArray.getJSONObject(i).getJSONObject("AddressDetails").getJSONObject("Country");
				JSONObject uu = u.getJSONObject("AdministrativeArea").getJSONObject("Locality");
	...
	public void getLocationByBase()
	{
		...
		try
		{
			...
			else if (type == TelephonyManager.PHONE_TYPE_CDMA || type == TelephonyManager.NETWORK_TYPE_EVDO_A || type == TelephonyManager.NETWORK_TYPE_1xRTT)
			{
		...
		try
		{
			...
			if (state == HttpStatus.SC_OK)
			{
				...
				if (entity != null)
				{
					...
					getAddress(String.valueOf(data.get("latitude")), String.valueOf(data.get("longitude")));
				}
				else
				{
					handler.sendEmptyMessage(MESSAGE_OVER);
