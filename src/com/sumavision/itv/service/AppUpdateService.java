...
public class AppUpdateService extends Service
{
	...
	protected void downLoadFile(String httpUrl)
	{
		...
		if (!sdCardExist)
		{
			Toast.makeText(this, "SD卡不存在，无法更新程序，请插入SD卡后重试", Toast.LENGTH_SHORT).show();
			handler.sendEmptyMessage(MESSAGE_OVER);
		}
		else
		{

			folderPath = Environment.getExternalStorageDirectory() + File.separator + "TVFan" + File.separator;
	...
	private void notifyProgress(int progressValue, String percent)
	{
		...
		String title;
		RemoteViews contentView = new RemoteViews(getPackageName(), R.layout.notification_app_view);
		Intent intent = new Intent();
		if (progressValue != fileSize)
		{
			icon = android.R.drawable.stat_sys_download;
			title = "下载中";
			contentView.setProgressBar(R.id.progressBar, fileSize, progressValue, false);
		...
		else
		{
			...
			contentView.setTextViewText(R.id.err_text, "ITV新版本下载完成,点击可以安装");
